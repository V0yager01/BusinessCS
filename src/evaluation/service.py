...
async def create_rate_and_change_task(task_uuid, rate):
...
async def get_all_evaluation_by_date(uuid, start_date, end_date):
    evaluationrepo = EvaluationRepo()
    try:
...
async def get_avg_rate_by_user_uuid(uuid):
    evaluationrepo = EvaluationRepo()
    try:
